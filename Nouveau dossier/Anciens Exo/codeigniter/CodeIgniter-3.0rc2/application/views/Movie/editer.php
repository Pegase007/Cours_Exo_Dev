...
<h1>Page editer movie</h1>

<!-- 6. $HORIZONTAL_FORM ===========================================================================

	Horizontal form
-->
	<form enctype='multipart/form-data' action="<?php echo site_url('movie/editer/'.$movie->id);?>" method='post' class="panel form-horizontal">
		<div class="panel-heading">
			<span class="panel-title fa fa-plus"> Modification de films</span>
		</div>
		<div class="panel-body">
			<div class="row form-group">
				<label class="col-sm-4 control-label">Titre du film:</label>
				<div class="col-sm-8">
					<input type="text" name="titre" value='<?php echo $movie->title; ?>' class="form-control" placeholder='Titre'>
			...
			<div class="row form-group">
				...
				<div class="col-sm-8">
					<input type="number" min='1950' max='2020' name="annee" value='<?php echo $movie->annee; ?>' class="form-control" placeholder='ex: 2010'>
			...
			<div class="row form-group">
				...
				<div class="col-sm-8">
					<input type="number" min='20000' max='999999999' name="budget" value='<?php echo $movie->budget; ?>' class="form-control" placeholder='Minimum 200000'>
			...
			<div class="row form-group">
				...
				<div class="col-sm-8">
					<input type="date" name="date" value='<?php echo $movie->date_release; ?>' class="form-control">
				...
				<div class='col-sm-8'>
					<div class='col-sm-4'>
						<select class="form-control" name='bo' value='<?php echo set_value("bo")?>'>
							<option value=''></option>
							<option value="vo" <?php if ($movie->bo == 'VO'){?>selected<?php } ?>>VO</option>
							<option value="vost" <?php if ($movie->bo == 'VOST'){?>selected<?php } ?>>VOST</option>
							<option value="vf" <?php if ($movie->bo == 'VF'){?>selected<?php } ?>>VF</option>
						</select>
					...
					<div class="col-sm-4">
							<label class="checkbox-inline">
								<input type="checkbox" name='visible' value="1" class="px" <?php if ($movie->visible == "1"){?>checked<?php } ?>> <span class="lbl">Visible</span>
							</label>
							<?php echo form_error('visible','<div class="alert alert-dark alert-danger">
						<button type="button" class="close" data-dismiss="alert">x</button>','</div>'); ?>
					
							<label class="checkbox-inline">
								<input type="checkbox" name='couv' value="1" class="px" <?php if ($movie->cover == "1"){?>checked<?php } ?>> <span class="lbl">En couverture</span>
							</label>
					...
					<p class="row form-group pull-right">
						...
						<label class="radio">
							<input type="radio" name="distrib" value='Warner_bros' <?php if ($movie->distributeur == "Warner_Bros"){?>checked<?php } ?> class="px">
		...
		<div class="row form-group">
			<label class="col-sm-4 control-label">Catégorie:</label>	
				<div class='col-sm-8'>
					<div class="row">
						<div class='col-sm-4'>
							<select class="form-control" name='catego'>
								...
								<option value="<?php echo $allcategorie->id ?>" <?php if ($movie->categories_id == $allcategorie->id){?>selected<?php } ?> ><?php echo $allcategorie->title ?></option>
								<?php }?>
							</select>
					...
					<div class="col-sm-5">
						<input type="file" id="image" capture='capture' name="image" class="form-control" placeholder='http://' value='<?php echo set_value("image")?>'>
						<?php echo form_error('image','<div class="alert alert-dark alert-danger">
						<button type="button" class="close" data-dismiss="alert">x</button>','</div>'); ?>
						<img class="col-md-3" src="<?php echo $movie->image ?>">
					</div>
				</div>
			</div>
		</div>
			...
			<div class="row form-group">
				...
				<div class="col-sm-8">
					<textarea type="text" name="synopsis" value='<?php echo set_value("synopsis")?>' class="form-control"><?php echo $movie->synopsis; ?></textarea>
			...
			<div class="row form-group">
				...
				<div class="col-sm-8">
					<textarea type="text" name="description" value='<?php echo set_value("description")?>' class="form-control"><?php echo $movie->description; ?></textarea>
			...
			<div class="row form-group">
				...
				<div class="col-sm-8">
					<textarea type="url" name="trailer" value='<?php echo set_value("trailer")?>' class="form-control"><?php echo $movie->trailer; ?></textarea>
		...
		<div class="panel-footer text-right">
			<button type='submit' class="btn btn-primary">Modifier ce film</button>
		</div>
