    ...
    <div class="historic">
        ...
        <?php
           //Récupération dans la base de donnée
            ...
            if (isset($_GET["admin"])){
                echo '<strong>New Message : </strong>'.$donnees['text'].'<br />';
                echo "<input type='hidden' name='id' value='" . $donnees['id'] . "'><button type='submit' name='delete'>x</button>";
                echo "<input type='textarea' name='edit' value='" . $donnees['text'] . "' ><button type='submit' name='update'>Update</button>.<br />";
            }
            else 
                {                
                    //echo '<strong>New Message : </strong>'.$donnees['text'].'<br />';
                    echo str_replace(':)', '<img src="img/emot2.png">', '<strong>New Message : </strong>'.htmlspecialchars($donnees['text']).'<br />');
